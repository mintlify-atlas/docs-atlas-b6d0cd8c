...
- `ai.embed` (span): the full length of the embed call. It contains 1 `ai.embed.doEmbed` span.
...
Many spans that use LLMs contain the following attributes:
...
Spans that correspond to individual LLM calls contain
...
Many spans that use embedding models contain the following attributes:
